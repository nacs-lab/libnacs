...
NACS_EXPORT() void HostSeq::init_run()
{
    ...
    m_first_bseq = true;
    m_running = false;
    std::copy(default_values.begin(), default_values.end(),
              values.begin() + nconsts + nglobals + nglobal_vals);
}
...
void HostSeq::update_values(const BasicSeq &bseq)
{
    ...
    std::copy(values.begin() + nconsts + nglobals + nglobal_vals,
              values.begin() + nconsts + nglobals + nglobal_vals + nchannels,
              start_values.begin());
    ...
    memcpy(m_chn_vals.data(), values.data() + nconsts + nglobals + nglobal_vals,
    ...
    auto get_chn_val = [&] (uint32_t chn, int64_t time) {
        ...
        if ((double)delta < plen)
            return p->ramp_func(delta, values.data());
        return m_chn_vals[chn - 1];
    };
    ...
    while (scan_start != pulses_end) {
        ...
        while (scan_start != pulses_end) {
            ...
            if (pulse_time < last_time ||
                ...
                if (pulse_time < 0)
                    throw Error(uint32_t(Error::Pulse) << 16 | NaCs::Seq::Pulse::NegTime,
                                Error::Pulse, scan_start->id, "Pulse time must be positive.");
            ...
            if (scan_start->len != uint32_t(-1)) {
                ...
                if (get_value(bseq, scan_start->len) <= 0) {
                    throw Error(uint32_t(Error::Pulse) << 16 | NaCs::Seq::Pulse::NonPosLen,
                                Error::Pulse, scan_start->id, "Pulse length must be positive");
                }
            }
    ...
    auto p = values.data() + nconsts + nglobals + nglobal_vals;
    // Fill the current values back in
    for (uint32_t i = 0; i < nchannels; i++)
        p[i].f64 = get_chn_val(i + 1, -1);
