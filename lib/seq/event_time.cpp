...
NACS_EXPORT() bool EventTime::normalize()
{
    ...
    for (; i < nterms; i++) {
        ...
        if (term.sign == Pos && !(int64_t(t + 0.5) > 0))
...
NACS_EXPORT() Var *EventTime::to_var(Env &env) const
{
...

    {
        ...
        auto v05 = llvm::ConstantFP::get(cgctx->T_f64, 0.5);
        for (auto &arg: f->args()) {
            llvm::Value *argv = builder.CreateFPToSI(builder.CreateFAdd(&arg, v05), T_i64);
            if (!v) {
                v = argv;
                continue;
            }
