...
struct Compiler::TimeVal {
    bool is_var;
    llvm::SmallVector<uint32_t,4> terms; // varid or timeval id
    int64_t offset;
};
...
void Compiler::prescan_times()
{
    ...
    std::map<std::vector<uint32_t>,llvm::SmallVector<const Var*,2>,TermKeyCompare> terms;
    for (auto &[var, use]: varuse) {
        ...
        terms[reduce_use].append(rep, var);
    }
    ...
    lookup_func_t create_intermediate = [&] (term_key_t *terms, size_t nterms) ->
        IntermediateResult* {
        if (nterms == 0)
            return nullptr;
        ArrayKey<term_key_t> key{terms, nterms};
        ...
        auto parent = create_intermediate(terms, nterms - 1);
        it = intermediates.emplace(std::vector<term_key_t>{terms, terms + nterms},
                                   IntermediateResult{parent, {terms[nterms - 1]}}).first;
        return &it->second;
    };
    ...
    // Since we are not tracking the use of each single terms,
    // we will not assign a time offset to those.
    ...
    for (auto &[terms, intermediate]: intermediates) {
        ...
        auto slot = m_timevals.size();
        auto &timeval = m_timevals.emplace_back(TimeVal{false, {}, rel_offset});
        if (intermediate.parent)
            timeval.terms.push_back(find_val(intermediate_slots, intermediate.parent));
        for (auto term: intermediate.terms)
            timeval.terms.push_back(find_val(term_slots, term));
    ...
    for (uint32_t i = 0; i < nevent_times; i++) {
        ...
        auto slot = m_timevals.size();
        m_timevals.emplace_back(TimeVal{false, {intermediate_slot}, rel_offset});
...
void Compiler::classify_times()
{
    ...
    for (uint32_t timeid = 0; timeid < ntimevals; timeid++) {
        auto &timeval = m_timevals[timeid];
        bool refglobal = false;
        bool refchannel = false;
        bool refmeasure = false;
        uint32_t owner = 0;
        auto &types = timeval.is_var ? m_var_types : m_timeval_types;
        auto &owners = timeval.is_var ? m_var_owners : m_timeval_owners;
        assert(types.size() == owners.size());
        for (auto refid: timeval.terms) {
            assert(refid < types.size());
            auto type = types[refid];
            switch (type) {
            case VarType::Global:
            case VarType::GlobalValue:
                refglobal = true;
            ...
            case VarType::BSeqDirect:
                refchannel = true;
                assert(owner == owners[refid] || owner == 0);
                owner = owners[refid];
                break;
            case VarType::BSeqMeasure:
            case VarType::BSeqNeedOrder:
        ...
        if (refmeasure) {
            m_timeval_owners[timeid] = owner;
            m_timeval_types[timeid] = VarType::BSeqNeedOrder;
        }
        else if (refchannel) {
            m_timeval_owners[timeid] = owner;
            m_timeval_types[timeid] = VarType::BSeqDirect;
        }
        else {
            assert(refglobal);
            (void)refglobal;
            nglobal_vals++;
            m_timeval_types[timeid] = VarType::GlobalValue;
        }
    }
...
void Compiler::assign_shared_times()
{
    ...
    for (uint32_t timeid = 0; timeid < ntimevals; timeid++) {
        auto &timeval = m_timevals[timeid];
        if (m_timeval_types[timeid] == VarType::GlobalValue) {
            auto slot = global_value_offset + global_value_id;
            assert(slot < m_host_seq.types.size());
            m_host_seq.types[slot] = HostSeq::Type::Int64;
            m_timeval_slots[timeid] = slot;
            auto &types = timeval.is_var ? m_var_types : m_timeval_types;
            auto &slots = timeval.is_var ? m_var_slots : m_timeval_slots;
            assert(types.size() == slots.size());
            for (auto refid: timeval.terms) {
                assert(refid < slots.size());
                auto refslot = slots[refid];
                assert(refslot < slot);
                auto reftype = types[refid];
...
void Compiler::assign_bseq_vars()
{
    ...
    for (uint32_t bseq_idx = 0; bseq_idx < nbseqs; bseq_idx++) {
        ...
        for (uint32_t timeid = 0; timeid < ntimevals; timeid++) {
            auto owner = m_timeval_owners[timeid];
            if (owner != seq_id)
                continue;
            switch (m_timeval_types[timeid]) {
            case VarType::BSeqDirect:
                ndirect++;
                break;
            case VarType::BSeqNeedOrder:
        ...
        auto add_dependency_timeid = [&] (uint32_t timeid) {
            if (m_timeval_types[timeid] != VarType::GlobalValue)
                return;
            add_dependency_global_val_id(m_timeval_slots[timeid] - global_value_offset);
        };
    ...
    for (uint32_t bseq_idx = 0; bseq_idx < nbseqs; bseq_idx++) {
        ...
        for (uint32_t timeid = 0; timeid < ntimevals; timeid++) {
            ...
            if (owner != seq_id)
                continue;
            auto type = m_timeval_types[timeid];
            auto &timeval = m_timevals[timeid];
            uint32_t bseq_slot;
...
void Compiler::cache_time_slots()
{
    ...
    for (uint32_t i = 0; i < nevent_times; i++) {
        auto et = m_event_times[i];
        auto timeid = i;
        auto slot = m_timeval_slots[timeid];
...
void Compiler::populate_bseqs()
{
    ...
    for (uint32_t bseq_idx = 0; bseq_idx < nbseqs; bseq_idx++) {
        ...
        for (auto &assume: bseq->get_assumes()) {
            ...
            assert(timeid < m_timeval_types.size());
            if (m_timeval_types[timeid] == VarType::BSeqNeedOrder)
                continue;
            auto slot = m_timeval_slots[timeid];
        ...
        for (auto &[chn, info]: bseq->get_channel_infos()) {
            for (auto &pulse: info.pulses) {
                ...
                if (pulse.is_measure()) {
                    host_pulse.len = -2;
                    host_pulse.value = -1;
                    host_pulse.measure = m_var_slots[pulse.val()->varid()];
                }
                else {
                    auto len = pulse.len();
                    ...
                    if (it == info.pulse_start_vars.end()) {
                        ...
                        host_pulse.measure = m_var_slots[it->second->varid()];
                    }
                }
...
void Compiler::generate_fptrs()
{
    ...
    std::map<val_eval_t*,llvm::Function*> val_funcs;
    std::map<ramp_eval_t*,llvm::Function*> ramp_funcs;
    ...
    for (uint32_t varid = 0; varid < nvars; varid++) {
        ...
        if (isramp) {
            for (auto [bseq_idx, pulse_id]: find_val(m_var_bseqidx_pulseid, varid)) {
                auto &host_bseq = m_host_seq.seqs[bseq_idx];
                ramp_funcs.emplace(&(host_bseq.pulses[pulse_id].ramp_func), wrapf);
            }
        }
        else {
            val_funcs.emplace(fptr_slot, wrapf);
        }
    }
    ...
    uint32_t ntimevals = m_timevals.size();
    for (uint32_t timeid = 0; timeid < ntimevals; timeid++) {
        auto fptr_slot = find_fptr_slot(timeid, false);
        if (!fptr_slot)
            continue;
        assert(!*fptr_slot);
        auto &timeval = m_timevals[timeid];
    ...
    for (auto [slot, f]: val_funcs) {
        f->setLinkage(llvm::GlobalValue::ExternalLinkage);
        f->setVisibility(llvm::GlobalValue::ProtectedVisibility);
    }
    for (auto [slot, f]: ramp_funcs) {
        f->setLinkage(llvm::GlobalValue::ExternalLinkage);
        f->setVisibility(llvm::GlobalValue::ProtectedVisibility);
    }
    ...
    if (!m_obj_id)
        throw std::runtime_error("Sequence function compilation failed.");
    for (auto [slot, f]: val_funcs) {
        auto sym = m_engine.get_symbol(f->getName());
        *slot = val_eval_t(sym);
    }
    for (auto [slot, f]: ramp_funcs) {
        auto sym = m_engine.get_symbol(f->getName());
        *slot = ramp_eval_t(sym);
    }
    m_engine.reset_dyld();
}
