      ...
      m_val(val->ref())
{
    ...
    // Measurement value must be extern
    assert(!is_measure || (val->is_extern() && !len));
...
NACS_EXPORT() EventTime::Sign Pulse::known_before(const Pulse &p2) const
{
    auto sign = start().isless(p2.start());
    if (sign == EventTime::NonNeg)
        sign = id() < p2.id() ? EventTime::Pos : EventTime::Unknown;
    return sign;
}
...
NACS_EXPORT() void Pulse::check_start() const
{
    if (m_start->terms.empty() && m_start->tconst < 0) {
        throw Error(uint32_t(Error::Pulse) << 16 | NegTime,
                    Error::Pulse, m_id, "Pulse time must be positive.");
    }
}
