...
struct HostSeq {
    ...
    llvm::ArrayRef<uint32_t> global_ages() const
    {
        return m_ages;
    }
    double get_value(uint32_t i) const
    {
        return get_value(seqs[m_cur_seq_idx], i);
    }
...
private:
    ...
    // Global age tracking
    uint32_t m_cur_age;
    std::vector<uint32_t> m_ages; // `.size() == nglobals + nglobal_vals`
    bool m_global_dirty;
