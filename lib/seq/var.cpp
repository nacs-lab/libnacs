...
static llvm::Value *convert_arg(llvm::IRBuilder<> &builder, llvm::Type *typ, llvm::Value *val)
{
    auto vt = val->getType();
    if (vt == typ)
        return val;
...
NACS_INTERNAL bool Var::reduce_args()
{
    ...
    try {
        ...
        for (size_t i = 0; i < nargs; i++) {
            call_args[i] = convert_arg(builder, oldfty->getParamType(i), call_args[i]);
        }
    }
    catch (...) {
...
bool Var::optimize_call()
{
    ...
    auto llvm_mod = m_env.llvm_module();
    auto &llvm_ctx = llvm_mod->getContext();
    auto temp_mod = std::make_unique<llvm::Module>("", llvm_ctx);
    // Copy the function and its dependencies to a temporary module for compilation.
    LLVM::FunctionMover mover(temp_mod.get());
