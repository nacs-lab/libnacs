...
void Backend::parse_data(const uint8_t *data, size_t len)
{
    ...
    for (auto i = 0; i < nttl_mgrs; i++) {
        ...
        int64_t min_time;
        if (ver == 0) {
            off_delay = reader.read<uint32_t>();
            on_delay = reader.read<uint32_t>();
            skip_time = reader.read<uint32_t>();
            min_time = reader.read<uint32_t>();
        }
        else {
            off_delay = reader.read<int64_t>();
            on_delay = reader.read<int64_t>();
            skip_time = reader.read<int64_t>();
            min_time = reader.read<int64_t>();
        }
        if (off_delay < 0 || on_delay < 0 || skip_time < 0 || min_time < 0)
            throw std::runtime_error(name() + ": Offsets for TTL manager "
