...
public:
    ...
    uint32_t assume_slot(const assume_entry_t *assume) const
    {
        // Mainly for testing
        // since the backend generally doesn't need to care about assumptions
        return m_timeval_slots[m_time_timevals[find_val(m_assumption_ids, assume)]];
    }
    const Seq *seq() const
    {
        return &m_seq;
    }
    llvm::ArrayRef<const BasicSeq*> basic_seqs() const
    {
        return m_bseqs;
    }
    static llvm::Function *get_ramp_func(Var *val, llvm::Function *f, llvm::Type *ret_ty,
                                         llvm::Type *time_ty, bool _export=false);
...
private:
    ...
    // Map from event time and assumption to index in
    // m_event_times, m_assumptions (with offset) and m_time_timevals.
