...
public:
    ...
    public:
        struct ChnOutput {
            ChnOutput(std::string name, int64_t *times, double *values, uint32_t *pulse_ids, size_t npts)
            : name(std::move(name)), times(times), values(values), pulse_ids(pulse_ids), npts(npts)
            {
                //
            }
    ...
    char *get_config_str(const std::string &name, size_t *sz);
