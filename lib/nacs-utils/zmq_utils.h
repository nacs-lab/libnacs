...
public:
    ...
    public:
        ...
        void send_msg(SendFunc &&send_func, finish_cb_t finish_cb, int64_t timeout_ms = -1)
        {
        ...
        template<typename SendFunc>
        std::future<std::vector<zmq::message_t>> send_msg(SendFunc &&send_func, int64_t timeout_ms = -1)
        {
