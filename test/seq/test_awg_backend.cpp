...
void test_set_pulse_global() {
    ...
    printf(expseq->host_seq.first_bseq ? "first_bseq" : "not_first_bseq");
    expseq->init_run();
    expseq->host_seq.set_global(0, 4e3);
    expseq->host_seq.set_global(1, 0.4);
    expseq->host_seq.set_global(2, 5e3);
    ...
    //printf("Is first bseq: %d\n", expseq->host_seq.first_bseq);
    printf(expseq->host_seq.first_bseq ? "first_bseq" : "not_first_bseq");
    expseq->host_seq.set_global(0, 3e3);
...
void test_set_pulse_global_mult() {
    ...
    stm.write<uint32_t>(1);
    {
        ...
        stm.write<uint32_t>(0); // cond
        stm.write<uint32_t>(2); // chn
        // 3 conditionally on global 4, 17 ms after start, set chn1 of out2 freq to global 2
        stm.write<uint32_t>(22); // id
        stm.write<uint32_t>(2); // time_id
        stm.write<uint32_t>(0); // len
        stm.write<uint32_t>(3); // val
        stm.write<uint32_t>(5); // cond
        stm.write<uint32_t>(3); // chn
        // 4 conditionally on global 4 17 ms after start, set chn1 of out2 amp to global 3
        stm.write<uint32_t>(75); // id
        stm.write<uint32_t>(2); // time_id
        stm.write<uint32_t>(0); // len
        stm.write<uint32_t>(4); // val
    ...
    expseq->host_seq.set_global(2, 5e3);
    expseq->host_seq.set_global(3, 0.1);
    expseq->host_seq.set_global(4, 1); // conditionally turn on chn 2 pulse
    expseq->pre_run();
    expseq->start();
    expseq->wait(60000);
    expseq->post_run();
    ...
    printf(expseq->host_seq.first_bseq ? "first_bseq" : "not_first_bseq");
    expseq->host_seq.set_global(0, 3e3);
    expseq->host_seq.set_global(4, 0);
    expseq->init_run();
    expseq->pre_run();
    expseq->start();
    expseq->wait(60000);
