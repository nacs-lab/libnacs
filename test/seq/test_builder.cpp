...
static void test_default(llvm::LLVMContext &llvm_ctx)
{
    ...
    assert(defval->is_const());
    assert(defval->is_const());
    assert(defval->get_const().is(IR::TagVal(2.3)));
    assert(defval == seq.get_const(IR::TagVal(2.3)));
}
...
static void test_time(llvm::LLVMContext &llvm_ctx)
{
    ...
    std::vector<const Seq::BasicSeq::Assumption*> assumes;
    for (auto &assume: bseq.get_assumes())
        assumes.push_back(&assume);

    assert(assumes.size() == 2);
    assert(assumes[0]->sign == Seq::EventTime::Pos);
    assert(assumes[0]->id == 101);
    assert(assumes[0]->val->is_const());
    assert(assumes[0]->val.get() == seq.get_const(IR::TagVal(4.2)));
    assert(assumes[1]->sign == Seq::EventTime::Pos);
    assert(assumes[1]->id == 220);
    assert(assumes[1]->val->is_const());
    assert(assumes[1]->val.get() == seq.get_const(IR::TagVal(1000.0)));
