...
struct CompileSeq {
    ...
    void test_host_seq()
    {
        assert(host_seq.nshared == host_seq.nconsts + host_seq.nglobals +
               host_seq.nglobal_vals + host_seq.nchannels);
        assert(host_seq.npublic_globals <= host_seq.nglobals);
        assert(host_seq.values.size() >= host_seq.nshared);
        assert(host_seq.default_values.size() == host_seq.nchannels);
        assert(host_seq.types.size() == host_seq.nconsts + host_seq.nglobals +
               host_seq.nglobal_vals);
        assert(host_seq.depends.size() == host_seq.nglobal_vals);
        for (auto &deps: host_seq.depends) {
            assert(!deps.empty());
            for (auto dep: deps)
                assert(dep < host_seq.nglobals);
            assert(std::is_sorted(deps.begin(), deps.end()));
        }
        assert(host_seq.global_evals.size() == host_seq.nglobal_vals);
        for (auto p: host_seq.global_evals)
            assert(p);
        assert(host_seq.seqs.size() == seq.get_basicseqs().size());
        for (auto &host_bseq: host_seq.seqs) {
            ...
            assert(host_seq.values.size() >= bseq_nvars);
            assert(host_bseq.types.size() == host_bseq.ndirect + host_bseq.nneed_order);
            assert(host_bseq.evals.size() == host_bseq.ndirect + host_bseq.nneed_order);
            for (auto p: host_bseq.evals)
                assert(p);
            assert(std::is_sorted(host_bseq.global_refs.begin(), host_bseq.global_refs.end()));
            for (auto ref: host_bseq.global_refs)
                assert(ref < host_seq.nglobal_vals);
            assert(std::is_sorted(host_bseq.cond_global_refs.begin(),
            ...
            assert(host_bseq.reverse_depends.size() == host_bseq.nmeasure);
            for (auto &rdeps: host_bseq.reverse_depends) {
                assert(std::is_sorted(rdeps.begin(), rdeps.end()));
                for (auto rdep: rdeps) {
                    assert(rdep < host_bseq.nneed_order);
                    deps_count[rdep] += 1;
                }
            }
            assert(host_bseq.deps_count.size() == host_bseq.nneed_order);
            assert(host_bseq.deps_count == deps_count);
            for (auto &assign: host_bseq.assignments) {
                assert(assign.global_id < host_seq.nglobals);
                assert(assign.value < bseq_nvars);
            }
            for (auto &assume: host_bseq.assumptions) {
                assert(assume.sign != Seq::EventTime::Unknown);
                assert(assume.value < bseq_nvars);
                if (assume.value >= host_seq.nshared + host_bseq.nmeasure + host_bseq.ndirect) {
                    ...
                    assert(assume_id != uint32_t(-1));
                    assert(&host_bseq.assumptions[assume_id] == &assume);
                }
            }
            for (auto &br: host_bseq.branches) {
                assert(br.cond < bseq_nvars);
                assert(br.target == uint32_t(-1) || br.target < host_seq.seqs.size());
            }
            assert(host_bseq.default_branch == uint32_t(-1) ||
                   host_bseq.default_branch < host_seq.seqs.size());
            for (auto et: host_bseq.endtimes)
                assert(et < bseq_nvars);
            assert(host_bseq.ndirect_assumes <= host_bseq.assumptions.size());
            assert(host_bseq.assumptions_idx.size() == host_bseq.nneed_order);
            uint32_t nneed_order_assume = 0;
            for (auto ai: host_bseq.assumptions_idx) {
                if (ai == uint32_t(-1))
                    continue;
                nneed_order_assume++;
                assert(ai < host_bseq.assumptions.size());
            }
            assert(host_bseq.ndirect_assumes + nneed_order_assume ==
